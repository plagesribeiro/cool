...
% union
{
  ...
  char *error_msg;
    ...
    257)
    */
    % token CLASS 258 ELSE 259 FI 260 IF 261 IN 262 % token INHERITS 263 LET 264 LOOP 265 POOL 266 THEN 267 WHILE 268 % token CASE 269 ESAC 270 OF 271 DARROW 272 NEW 273 ISVOID 274 % token<symbol> STR_CONST 275 INT_CONST 276 % token<boolean> BOOL_CONST 277 % token<symbol> TYPEID 278 OBJECTID 279 % token ASSIGN 280 NOT 281 LE 282 ERROR 283
    ...
    % type<program> program % type<classes> class_list % type<class_> class

    /* You will want to change the following line. */
    % type<feature> feature % type<features> features % type<formal> formal % type<formals> formals % type<case_> branch % type<cases> branches % type<expression> expression % type<expression> block_expression % type<expressions> block_expressions % type<expressions> arguments_expressions % type<expression> dispatch_expression % type<expression> let_expression % type<expression> let_assign % type<feature> method_declaration % type<feature> attribute_declaration

    % right ASSIGN % precedence NOT % nonassoc '<' '=' LE % left '+' '-' % left '*' '/' % precedence ISVOID % precedence '~' % precedence '@' % precedence '.' % %
    /*
...
method_declaration : OBJECTID '(' formals ')' ':' TYPEID '{' expression '}'
{
  @$ = @9;
  $$ = method($1, $3, $6, $8);
};
...
let_expression : OBJECTID ':' TYPEID let_assign IN expression
{
  @$ = @6;
  $$ = let($1, $3, $4, $6);
}
